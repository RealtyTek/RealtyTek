...
class CmsUser extends Authenticatable
{
     ...
     */
    protected $fillable = [
        'cms_role_id','user_ref_id', 'name', 'username', 'slug', 'email', 'mobile_no', 'password', 'image_url','is_email_verify', 'status', 'remember_token',
    ...
    public static function updateCmsUser($user_id)
    {
        self::where('user_ref_id',$user_id)->update(['is_email_verify'=>'1']);
    ...
    public static function CreateCmsAgent($data){
        self::create([
            'cms_role_id'=>2,
            'user_ref_id'=>$data->id,
            'name'=>$data->name,
